...
const cartCount = document.querySelector("#update-cart")
...
window.addEventListener("DOMContentLoaded", () => {
  if (!isGerenteAtual() && linkFuncionarios) {
...
if (linkFuncionarios) {
  linkFuncionarios.addEventListener("click", (event) => {
    ...
    if (!isGerenteAtual()) {
      alert("Acesso permitido apenas para gerentes")
      return
    }
...
document.addEventListener("click", (event) => {
  if (event.target.closest("#open-cart")) {
    cartAside.classList.add("active")
  }
...
function renderPageFuncionarios() {
  ...
  document.querySelector("#main-content").append(sectionFuncionario())
}

function renderPageProducts() {
  limparConteudoSemApagarCarrinho()
  document.querySelector("#main-content").append(sectionProduct())
}
...
function submitFormFuncionario() {
  ...
  formFuncionario.addEventListener("submit", async (event) => {
    ...
    if (!nome || !tipo || tipo === "selecionar o cargo") {
...
function newFuncionario(funcionario) {
  ...
  divRemove.addEventListener("click", async () => {
    const confirmar = confirm("Tem certeza que deseja remover este funcionário?")
    if (!confirmar) return

    try {
      await window.apiRequest(`/workers/${funcionario.worker_id}`, {
        method: "DELETE"
      })
...
function newProduct(product) {
  ...
  if (product.estoque < 50) {
    stock.classList.add("baixo")
  } else if (product.estoque < 100) {
  ...
  const buttonStock = createButton("+ Estoque")
  buttonStock.classList.add("add")
  ...
  buttonRemove.addEventListener("click", async () => {
    ...
    try {
      await window.apiRequest(`/products/${product.product_id}`, {
        method: "DELETE"
      })
  ...
  divButtons.append(buttonAdd, buttonStock, buttonRemove)
...
function aumentarQuantidade(product_fk) {
  const carrinho = JSON.parse(localStorage.getItem("carrinho")) || []
  const item = carrinho.find((item) => item.product_fk === product_fk)
  const product = products.find((product) => product.product_id === product_fk)
...
async function reporEstoque(product) {
  const quantidade = Number(prompt("Quantidade para adicionar ao estoque:"))
  ...
  const novoEstoque = Number(product.estoque) + quantidade
